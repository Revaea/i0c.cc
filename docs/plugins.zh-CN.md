...
有些值必须在读取远程文档之前存在。GitHub 所有者、仓库、分支、路径、OAuth scope、初始 Raw URL 和已安装 Runtime 平台包因此属于**启动配置**，不是远程插件配置。它们位于 `@i0c/config` 或根目录 `i0c.runtime.config.ts` 安装清单，修改后需要重新构建。Git 与 Runtime Manifest 会明确拒绝在 `plugins.*.config` 中填写这些启动字段，避免出现“能通过校验，却无法初始化自身加载器”的假配置。
...
Runtime 平台属于编译期插件，但不会硬编码在 `apps/runtime` 中。平台包导出 `./manifest`、`./runtime` 与 `./installation`；Runtime 入口导出 `runtimePlatformPlugin`，Installation 入口声明包模块、需要打包的依赖、Provider 标识、构建键和输出路径。根目录 `i0c.runtime.config.ts` 是平台安装清单。

`apps/runtime/src/entry.ts` 只导入由 `@i0c/runtime-build` 生成的虚拟所选平台模块。构建时会注入一个所选适配器和全部已安装平台 Manifest，`@i0c/runtime-host` 再把所选插件 ID 与受约束的 Provider 标识加入请求上下文。因此新增第三方适配器只需安装包、把 Installation 描述符加入根清单，并提供部署配置；无需修改 `apps/runtime`、统计事件类型或官方 Catalog。
...
4. 官方非平台插件需要在对应 Catalog 预设中注册 Manifest。Runtime 平台改为导出 `./installation` 并加入 `i0c.runtime.config.ts`；第三方 Runtime 平台无需修改官方 Catalog。
